...
namespace Familia.Services {
    ...
    internal class SmartBandService : Service {
        ...
        private readonly Location.LocationManager location = Location.LocationManager.Instance;
        ...
        private void GetSteps() {
            location.LocationRequested += RequestedLocationForSteps;
        }
        private void RequestedLocationForSleep(object source, EventArgs args) {
            Task.Run(async () => {
                ...
                try {
                    ...
                    if (!Utils.CheckNetworkAvailability()) {
                        ...
                        Log.Error("FitBit Latitude", (args as LocationEventArgs).Location.Latitude.ToString().Replace(',', '.') + "");
                        Log.Error("FitBit Longitude", (args as LocationEventArgs).Location.Longitude.ToString().Replace(',', '.') + "");

                        obj
                        .Put("imei", Utils.GetDeviceIdentificator(this))
                        .Put("idClient", Utils.GetDefaults("IdClient"))
                        .Put("idPersoana", Utils.GetDefaults("IdPersoana"))
                        .Put("data", jsonArray)
                        .Put("latitude", (args as LocationEventArgs).Location.Latitude.ToString().Replace(',', '.'))
                        .Put("longitude", (args as LocationEventArgs).Location.Longitude.ToString().Replace(',', '.'));
            ...
            location.LocationRequested -= RequestedLocationForSleep;
        }
