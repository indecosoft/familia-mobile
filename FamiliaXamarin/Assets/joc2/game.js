...
function onDropInside(dashedElement, uiElement) {
    ...
    const error = 7;
    uiElement.css({ left: dashed.center.x - (child.width / 2) - error, top: dashed.center.y - (child.height / 2) - error, position: 'absolute' });
    updateCurrentPostion(uiElement, child.center.x, child.center.y);

    //TODO fix - when you move a dashed into another dashed element, the behaviour is not ok
    ...
    if (dashedElements[idElement].occupied.element == uiElement) {
        ...
        try {
            setElementToOldPosition(dashedElements[idElement].occupied.element, getPosition(dashedElements[idElement].occupied.element, children));
        } catch (e) { }
...
function onDropOutside(ui) {
    ...
    if (idElement != -1) {
        updateCurrentPostion(ui.helper, children[idElement].oldPosition.x, children[idElement].oldPosition.y);
        ...
        for (let i = 0; i < dashedElements.length; i++) {
            try {
                ...
                if (children[idx].oldPosition.x == children[idx].currentPosition.x && children[idx].oldPosition.y == children[idx].currentPosition.y) {
                    console.log('Important 3 item is still in the dashedelements but his location is at old position');
...
function setElementToOldPosition(ui, idElement) {
    ...
    const error = 7;
    ui.css({ left: child.center.x - (child.width / 2) - error, top: child.center.y - (child.height / 2) - error, position: 'absolute' });
}
